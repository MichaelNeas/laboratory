...
class Solution {
    func numUniqueEmails(_ emails: [String]) -> Int {
        ...
        return emails.reduce(0, { prev, curr in
            var localFormatted = ""
            var ignore = false
            for (index, local) in curr.enumerated() {
                if local == "@" {
                    let start = curr.index(curr.startIndex, offsetBy: index)
                    let range = start..<curr.endIndex
                    localFormatted.append(String(curr[range]))
                    if emailSet.contains(localFormatted) {
                        return prev
                    } else {
                        emailSet.insert(localFormatted)
                        return prev + 1
                    }
                }
                if !ignore {
